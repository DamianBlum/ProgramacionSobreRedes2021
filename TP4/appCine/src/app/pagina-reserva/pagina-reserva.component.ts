...
export class PaginaReservaComponent implements OnInit {
  ...
  ConseguirButacasDisponiblesYNo(
    butacas: String,
    butacas_disponibles: String
  ): void {
    butacas = butacas
      .replace(/"/g, '')
      .replace(']', '')
    ...
    this.conf.getCartelera().subscribe((dataPeli) => {
      dataPeli.forEach((peli) => {
        if (peli.id == this.route.snapshot.paramMap.get('id')) {
          this.pelicula = peli;
          this.ConseguirButacasDisponiblesYNo(
            peli.butacas.toString(),
            peli.butacas_disponibles.toString()
          );
        }
      });
    });
  ...
  reservar(): void {
    ...
    this.conf.reservarButacas(
      this.pelicula.id,
      butacas_seleccionadas.toString()
    );
  }
}
