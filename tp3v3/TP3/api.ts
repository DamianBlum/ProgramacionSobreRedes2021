...
if(cluster.isWorker){
    ...
    app.post('/cancelar_reserva', (req,res) =>{
        ...
        let id_reserva:number=req.body.id_reserva;
        ...
        if(error) throw error;
            ...
            await pool.query(`SELECT butacas_disponibles FROM funciones inner join reservas on funciones.id=reservas.funcion WHERE reservas.id=${id_reserva}`, async (error2,results2) =>{
